...
void addEdge(visualization_msgs::MarkerArray& markerArrayMsg, geometry_msgs::Point& vertex0, geometry_msgs::Point& vertex1)
{
    static int id = 0;
    ...
    edgeMarker.scale.x = getSegmentSize(vertex0, vertex1);
...
int main(int argc, char **argv)
{
    ...
    // Initializes ROS
    ros::init(argc, argv, "TFToPathConverter_Markers");
    ros::NodeHandle node;
    ...
    while (node.ok())
    {
        ...
        {
            addEdge(markerArrayMsg, lastPoseMsg.pose.position, poseMsg.pose.position);
        }
